...
Mochi Reader не пытается универсально «угадать» произвольный сайт. Источник подключается только через один из двух декларативных контрактов. Оба реализуют один поток: `searchManga → getChapters → getChapterPages → standard Manga Reader`.
...
## Вариант 2: generic HTML adapter
