...
Адаптер вызывает `GET /chapter` с `manga={remoteId}`, языками `ru` и `en`, `includes[]=scanlation_group`, лимитом 100 и последовательными offset до окончания результата. Максимум — 5 000 глав, что совпадает с внутренним лимитом Mochi Reader.
...
- origin находится в явно разрешённом списке MangaDex image origins;
...
Разрешённые origin задаются в built-in config, а не принимаются от пользователя. Минимальный список:

- `https://api.mangadex.org` для JSON;
- `https://uploads.mangadex.org` для обложек и страниц.

Если MangaDex@Home вернёт другой origin, страница блокируется безопасно и ошибка сообщает, что CDN origin не разрешён. Новый origin добавляется только изменением и тестированием приложения.
